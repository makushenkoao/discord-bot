...
client.distube
    .on('addList', async (queue, playlist) => {
        // Emitted after bot add a new playlist to the playing Queue.
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .setFooter({
                ...
                iconURL: playlist.songs[0].user.displayAvatarURL({
                    size: 1024,
                }),
            });
    ...
    .on('addSong', async (queue, song) => {
        // Emitted after bot add a new song to the playing Queue.
    ...
    .on('empty', async (queue) => {
        // Emitted when there is no user in the voice channel and there is a playing queue. If there is no playing queue, it will leave the channel without emitting this event.
    ...
    .on('error', async (textChannel, error) => {
        // Emitted when bot encounters an error while playing songs.
    ...
    .on('noRelated', async (queue) => {
        // Emitted when autoplay is on, queue is empty, and bot cannot find related songs to play.
    ...
    .on('searchCancel', async (message) => {
        // Emitted when the search canceled due to timeout.
    ...
    .on('searchInvalidAnswer', async (message) => {
        // Emitted when the search canceled due to user's next message is not a number or out of results range.
    ...
    .on('searchNoResult', async (message) => {
        // Emitted when bot cannot find any results for the query.
    ...
    .on('searchResult', async (message, result) => {
        // Emitted when song param of play is invalid url. bot will wait for user's next message to choose a song manually.
